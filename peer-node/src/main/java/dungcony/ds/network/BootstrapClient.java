 ...
 * <p>Luồng chính:</p>
 * <pre>
 *   REGISTER → lưu user vào DB bootstrap
 *   JOIN     → đánh dấu online, nhận danh sách peer + offline messages
 *   LEAVE    → đánh dấu offline
 *   LIST     → lấy danh sách peer đang online
 * </pre>
 */
@Slf4j
public class BootstrapClient implements BootstrapGateway {
    ...
    public JoinResponse joinOrNull(PeerInfo peerInfo) {
        ...
        try {
            ...
            log.info("Bootstrap JOIN nhận sốPeer={}, tinOffline={}",
                    joinResponse.getOnlinePeers().size(), joinResponse.getOfflineMessages().size());
            return joinResponse;
    ...
    public boolean storeOffline(OfflineMessage message) {
        ...
        boolean success = "OK".equalsIgnoreCase(response);
        log.info("Kết quả Bootstrap STORE_OFFLINE={}, messageId={}, receiverId={}",
                success, message.messageId(), message.receiverId());
        return success;
    }
    ...
    public boolean createGroup(Group group, String createdBy) {
        ...
        boolean success = "OK".equalsIgnoreCase(response);
        log.info("Kết quả Bootstrap CREATE_GROUP={}, groupId={}, response={}",
                success, group.getGroupId(), response);
        return success;
    }
    ...
    private String requestRaw(String command, String payload) {
        ...
        try (Socket socket = new Socket()) {
            ...
            log.warn("Request bootstrap thất bại. command={}, địaChỉ={}:{}, lỗi={}",
                    command, host, port, e.getMessage());
            return null;
        }
    }
}
